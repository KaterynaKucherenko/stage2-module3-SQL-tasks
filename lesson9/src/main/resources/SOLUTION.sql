...
SELECT s.id, s.name FROM student AS s JOIN payment AS p ON s.id=p.student_id WHERE p.payment_date BETWEEN '2019-01-01' AND '2019-12-31' GROUP BY (student.id, student.name) HAVING COUNT(payment.amount) >2;
